...
if(length(args)==0) {
  ...
  cat('[method]\n')
  cat('--alpha         nominal alpha level of TDPs, default is 0.05 (%95 confidence)\n')
  cat('[output]\n')
  cat('--outfile       optional output file of TDP values (nifti)\n')
  cat('--outtable      optional output table of TDP values (txt)\n')
  cat('--intable       optional input table (txt) of clusters (TDPs will be added with _tdp)\n')
  cat('--inhtml        optional input html cluster file (TDPs will be added with _tdp)\n')
  cat('[options]\n')
...
if(valid) {
  ...
  if(length(grep('--tstat',args)) == 1) {
    ...
    if(class(tnifti)[1]=='try-error') stop('T-statistics file not found\n')
    tstat <- tnifti[]
    statdata <- tstat
  ...
  #call ARIBbrain
  ari_out <- ARIbrain::ARI(Pmap = pvalues2, clusters = cstat, mask = mask, alpha = ari_alpha, Statmap = statdata)
